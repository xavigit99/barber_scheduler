    ...
    <div>
      ...
      <div className="mb-8">
        ...
        <Table
          columns={[
            ...
            { key: 'start', header: 'Início', render: (w) => w.start_time },
            { key: 'end', header: 'Fim', render: (w) => w.end_time },
            {
              key: 'actions',
              header: '',
              render: (w) => <Button size="sm" variant="danger" onClick={() => handleDeleteWindow(w.id)}>Remover</Button>,
            },
          ]}
          data={windows}
          keyExtractor={(w) => w.id}
        ...
        <Table
          columns={[
            ...
            { key: 'motivo', header: 'Motivo', render: (b) => b.reason ?? '-' },
            {
              key: 'actions',
              header: '',
              render: (b) => <Button size="sm" variant="danger" onClick={() => handleDeleteBlock(b.id)}>Remover</Button>,
            },
          ]}
          data={blocks}
          keyExtractor={(b) => b.id}
      ...
      <Modal open={winModalOpen} onClose={() => setWinModalOpen(false)} title="Nova Janela Semanal">
        <form onSubmit={handleAddWindow} className="space-y-4">
          <Select label="Dia da Semana" options={WEEKDAYS} value={winForm.dia_semana} onChange={(e) => setWinForm({ ...winForm, dia_semana: e.target.value })} />
          <Input label="Hora Início" type="time" step="1" value={winForm.hora_inicio.slice(0, 5)} onChange={(e) => setWinForm({ ...winForm, hora_inicio: e.target.value + ':00' })} required />
          <Input label="Hora Fim" type="time" step="1" value={winForm.hora_fim.slice(0, 5)} onChange={(e) => setWinForm({ ...winForm, hora_fim: e.target.value + ':00' })} required />
      ...
      <Modal open={blockModalOpen} onClose={() => setBlockModalOpen(false)} title="Novo Bloqueio">
        <form onSubmit={handleAddBlock} className="space-y-4">
          ...
          />
          <Input label="Início" type="datetime-local" value={blockForm.inicio} onChange={(e) => setBlockForm({ ...blockForm, inicio: e.target.value })} required />
          <Input label="Fim" type="datetime-local" value={blockForm.fim} onChange={(e) => setBlockForm({ ...blockForm, fim: e.target.value })} required />
          <Input label="Motivo (opcional)" value={blockForm.motivo} onChange={(e) => setBlockForm({ ...blockForm, motivo: e.target.value })} />
          <div className="flex justify-end gap-2">
