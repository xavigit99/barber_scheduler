...
async def create_barber_availability(
    ...
    try:
        return await mediator.send(build_create_barber_availability_command(barber_id, payload))
...
async def list_barber_availabilities(
    ...
    mediator = build_mediator(db)
    try:
        return await mediator.send(build_list_barber_availabilities_query(barber_id))
...
async def update_barber_availability(
    ...
    try:
        return await mediator.send(
            build_update_barber_availability_command(barber_id, availability_id, payload)
        )
    except (ConflictError, NotFoundError, ValidationError) as exc:
        raise to_http_exception(exc) from exc
...
async def delete_barber_availability(
    ...
    deleted = await mediator.send(
        build_delete_barber_availability_command(barber_id, availability_id)
    )
...
async def create_barber_block(
    ...
    mediator = build_mediator(db)
    try:
        return await mediator.send(build_create_barber_block_command(barber_id, payload))
...
async def list_barber_blocks(
    ...
    mediator = build_mediator(db)
    try:
        return await mediator.send(build_list_barber_blocks_query(barber_id))
    except NotFoundError as exc:
        raise to_http_exception(exc) from exc
...
async def delete_barber_block(
    ...
    mediator = build_mediator(db)
    deleted = await mediator.send(build_delete_barber_block_command(barber_id, block_id))
